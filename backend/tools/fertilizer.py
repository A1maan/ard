...
def get_fertilizer_recommendations():
    ...
    # Get the path to the directory where crops.py is (backend/tools/)
    current_dir = Path(__file__).parent

    # Go up one level to 'backend' and find the file
    # current_dir.parent refers to: C:\Users\SAADB\Desktop\Python_Code\ard-new\ard\backend
    file_path = current_dir.parent / "farm_value_file.txt"
...
if __name__ == "__main__":
    ...
    # Simulate a BigQuery row (as would come from your ML predictions)
    example_bigquery_row = {
        "pred_ph.h2o_usda.a268_index": 5.2,
        "pred_n.tot_usda.a623_w.pct": 0.08,
        "pred_p.ext_usda.a1070_mg.kg": 12,
        "pred_k.ext_usda.a1065_mg.kg": 85,
        "pred_oc_usda.c729_w.pct": 0.8,
        "pred_ec_usda.a364_ds.m": 0.5,
        "pred_ca.ext_usda.a1059_mg.kg": 600,
        "pred_mg.ext_usda.a1066_mg.kg": 45,
        "pred_cec_usda.a723_cmolc.kg": 8,
        "pred_clay.tot_usda.a334_w.pct": 18,
        "pred_sand.tot_usda.c60_w.pct": 55,
        "pred_fe.ox_usda.a60_w.pct": 0.4,
        "pred_al.ox_usda.a59_w.pct": 0.3,
    }
    
    print("=" * 70)
    print("FERTILIZER RECOMMENDATION TOOL - MVP")
    print("=" * 70)
    
    result = get_fertilizer_recommendations(example_bigquery_row, target_crop="maize")
    
    print(f"\n📊 FARM SOIL HEALTH: {result['farm_soil_health']['overall_status'].upper()}")
    print(f"   Critical issues: {result['farm_soil_health']['critical_issues_count']}")
    print(f"   Moderate issues: {result['farm_soil_health']['moderate_issues_count']}")
    
    print("\n📋 SOIL ANALYSIS:")
    for key, value in result['soil_analysis'].items():
        if isinstance(value, dict) and 'value' in value:
            status = value.get('status', '')
            print(f"   {key}: {value['value']} {value.get('unit', '')} [{status}]")
    ...
    for rec in result['recommendations']:
        print(f"\n   [{rec['priority']}] {rec['nutrient']} - {rec['status']}")
        print(f"       Current: {rec['current_value']} → Target: {rec.get('target', 'N/A')}")
        print(f"       Products:")
        for prod in rec.get('products', [])[:2]:
            print(f"         • {prod['name']} ({prod.get('analysis', '')}) - {prod['rate']}")