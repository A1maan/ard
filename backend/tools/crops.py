...
if __name__ == "__main__":
    ...
    # Read from CSV
    csv_path = "/mnt/user-data/uploads/ossl_test_union_with_predictions.csv"
    
    print("=" * 60)
    print("CROP RECOMMENDATION TOOL - MVP")
    print("=" * 60)
    
    # Read all rows
    all_soils = read_all_rows_from_csv(csv_path)
    print(f"\nLoaded {len(all_soils)} soil samples from CSV\n")
    
    # Process first 3 samples as examples
    for i, soil in enumerate(all_soils[:3]):
        print(f"\n{'='*60}")
        print(f"SAMPLE {i+1} (Row ID: {soil['row_id']})")
        print(f"{'='*60}")
        
        result = get_crop_recommendations(soil)
        
        print(f"\nLocation: {result['location']}")
        print(f"\nSOIL SUMMARY:")
        print(f"  pH: {result['soil_summary']['ph']}")
        print(f"  EC: {result['soil_summary']['ec_dsm']} dS/m")
        print(f"  Texture: {result['soil_summary']['texture']['class']}")
        print(f"  Drainage: {result['soil_summary']['drainage']}")
        print(f"  Organic Carbon: {result['soil_summary']['organic_carbon_pct']}%")
        ...
        for crop in result['top_5_crops']:
            print(f"  {int(crop['score']):3d} | {crop['rating']:18s} | {crop['crop']}")
            if crop['issues']:
                print(f"       Issues: {', '.join(crop['issues'][:2])}")